...
const story = [
    {
        ...
        bg: "bg-school",
        showMatt: false, showMilca: false,
        next: 1
    },
    {
        name: "Matt",
...
document.addEventListener('keydown', e => {
    if (e.code === 'Space' && splash.classList.contains('hidden') === false) {
...
function showStep(step) {
    if (typeof step === 'string') return showEnding(step);
    const data = story[step];
    if (!data) return;

    bg.className = data.bg;
...
function showEnding(type) {
    ...
    if (type === "end_sad") {
        ...
        endingText.innerHTML = Ang araw na iyon ay naging pinakamasakit sa buhay ni Matt.<br><br>Hindi na muling nakausap si Milca. Ang kanyang pagmamahal ay naging alaala na lamang.<br>Sa bawat paglubog ng araw, laging naaalala niya ang babaeng minahal niya ngunit hindi niya naprotektahan.<br><br><em>Ang pagkakataon ay dumadaan lamang minsan. Huwag hayaang mawala bago pa man masabing mahal mo siya.</em>;
    } else {
        endingTitle.textContent = "MASAYANG WAKAS";
        endingText.innerHTML = Sa ilalim ng magandang paglubog ng araw sa tabi ng dagat, nagpalitan ng pangako sina Matt at Milca.<br><br>Ikinasal sila at namuhay nang masaya. Ang kanilang pagmamahalan ay naging patunay na ang pag-ibig na pinaghahandaan ay nagbubunga ng ligaya.<br>Ang paaralan, gym, at Jollibee ay mananatiling alaala ng simula ng kanilang walang hanggang pagmamahalan.<br><br><em>Ang tunay na pag-ibig ay hinihintay, pinag-iisipan, at pinahahalagahan.</em>;
    }
}
